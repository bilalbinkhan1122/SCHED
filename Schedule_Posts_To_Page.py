...
def random_request_id():
    first_char = random.choice("123456789")  # First character must be a digit
    second_char = random.choice(string.ascii_lowercase  + "123456789")  # Second can be a digit or letter
...
def login():
 global fb_dtsg, coki, page_access_token, pgid, token, uid
 global lsd, haste_session, __spin_r, __spin_t, __spin_b, hsi
 ...
 while True:
   ...
   lsd = re.search(r'\["LSD",\[],{"token":"(.*?)"', response.text)[1]
   haste_session = re.search('"haste_session":"(.*?)"', response.text)[1]
   __spin_r = re.search('"__spin_r":(.*?),', response.text)[1]
   __spin_t = re.search('"__spin_t":(.*?),', response.text)[1]
   __spin_b = re.search('"__spin_b":"(.*?)"', response.text)[1]
   hsi = re.search('"hsi":"(.*?)"', response.text)[1]
 ...
 actlink = response.headers['Location']
 ...
 response = requests.get(actlink,  headers=headers, proxies=proxies)
 ...
 response = requests.get('https://graph.facebook.com/v21.0/me/accounts?access_token='+token+'&fields=access_token,id,name,picture,is_published&limit=999',  headers=headers, proxies=proxies)
    ...
    for attempt in range(3):  
        try:
            ...
            response = requests.post(
                f'https://graph.facebook.com/{pgid}/photos',
                params=params,
                headers=headers,
                json=json_data,
                proxies=proxies,
...
def build_idnlink(encodeurl, postid):
    return (
        '%7B%22photo%22%3A%7B%22photo_link_metadata%22%3A%7B%22link%22%3A%7B%22external%22%3A%7B'
        f'%22url%22%3A%22{encodeurl}%22%7D%7D%7D%2C%22id%22%3A%22{postid}%22%7D%7D'
    )




def safe_post_with_retries(url, headers, data, retries=3, delay=2, timeout=20):
    for attempt in range(1, retries + 1):
        try:
            response = requests.post(url, headers=headers, data=data, proxies=proxies, timeout=timeout)
            return response
        except SSLError as e:
            print(f"{RED}[SSL ERROR] Attempt {attempt}/{retries} - {e}{RESET}")
        except RequestException as e:
            print(f"{RED}[REQUEST ERROR] Attempt {attempt}/{retries} - {e}{RESET}")
        time.sleep(delay * attempt)  # Backoff: 2s, 4s, 6s
    return None
...
def schedule_post(linkk, message, type, albumidd, pageid, actorid, timestamp, directlink, method):
    try:
        ...
        raw_album_id = re.search(r'https://www.facebook.com/(.*)', albumidd)[1]
        albumid = raw_album_id

        if method == '1':
            if pageid:
                albumid = f'{pageid}_{raw_album_id}'
            elif actorid:
                albumid = f'{actorid}_{raw_album_id}'

        elif method == '2' and pageid:
            albumid = f'{pageid}_{raw_album_id}'

        idnlink = None

        # Handle Album
        if 'album' in type.lower():
            idnlink = getalbumlinks(albumid)
            if idnlink is None:
                albumid = raw_album_id  # fallback to raw
                idnlink = getalbumlinks(albumid)
                method = '2'
                if idnlink is None:
        ...
        # Handle Photo
            ...
            link = linkk
            if directlink.lower() in ['yes', 'y']:
                link = getsrclink(albumid) or getsrclink(raw_album_id)
                if link is None:
                    print(f'{RED}FAILED TO GET SRC LINKS FROM : https://www.facebook.com/{raw_album_id}{RESET}')
                    return

            else:  # directlink = 'no' or 'n'
                postid = fetch_id(linkk)
                if postid:
                    encodeurl = encode_url(linkk)
                    idnlink = build_idnlink(encodeurl, postid)
                else:
            ...
            # Fallback if idnlink not yet built
            if not idnlink:
        ...
        message = add_before_after_message(message).replace('\n', '\\n').replace('"', '').replace('&', '%26')
        ...
        headers = {
            ...
            'Sec-Fetch-Dest': 'empty',
            'Connection': 'close',
            'Referer': f'https://business.facebook.com/latest/composer/?ref=biz_web_home_create_post&asset_id={pgid}&nav_ref=internal_nav&context_ref=HOME',
            'Accept-Language': 'en-US,en;q=0.9',
            'Cookie': coki,
        }

        data_string = build_data_string(group_ids, post_in_group, post_in_page, message, timestamp, idnlink, client_mutation_id)
        response = safe_post_with_retries(url='https://business.facebook.com/api/graphql/', headers=headers, data=data_string)

        if response is None:
            print(f"{RED}All retries failed for {albumidd}{RESET}")
            return None
...
def process_requests(file_path, start_time_str, delay):
    ...
    # Convert NaN in 'Message' column to empty string
    df['Message'] = df['Message'].fillna("")

    # Extract optional 'Page Id' and 'Actor Id' columns
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        # Schedule tasks
        for idx, (link, message, type, albumid, pageid, actorid) in enumerate(
                zip(links, messages, types, perlinks, 
                    pageids if pageids is not None else [None] * len(links), 
                    actorids if actorids is not None else [None] * len(links))):

            unix_timestamp = int(current_time.timestamp())
            future = executor.submit(
                schedule_post, link, message, type, albumid, pageid, actorid, 
                unix_timestamp, directlink, method)
            futures[future] = idx
